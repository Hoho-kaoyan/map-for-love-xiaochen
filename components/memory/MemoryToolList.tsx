...
type MemoryToolListProps = {
  config: ToolConfig;
  /** When provided, list items whose `date` equals this string are hidden.
   *  Used by `AnniversaryPage` to avoid duplicating the date already shown
   *  in the hero card. */
  excludeDate?: string;
};

/**
 * Body content shared by the four `MemoryToolPage` kinds (favorites,
 * anniversaries, time-capsule, trips). Owns its own state — items, form
 * fields, modal open, pagination. Excludes the page shell and title row so
 * callers (notably `AnniversaryPage`) can compose the body alongside other
 * content (e.g. a hero card) under a single `MemoryToolShell`.
 *
 * Behavior matches the original `MemoryToolPage.tsx` verbatim.
 */
export function MemoryToolList({ config, excludeDate }: MemoryToolListProps) {
  ...
  const visibleItems = excludeDate
    ? items.filter((item) => item.date !== excludeDate)
    : items;
  const visibleTotal = visibleItems.length;
  const visibleTotalPages = Math.ceil(visibleTotal / ITEMS_PER_PAGE);
      ...
      <header className="flex flex-wrap items-start justify-between gap-5">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[34px] font-semibold leading-tight text-[#5A6670]">
              {config.title}
            </h1>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#5A6670]/20 p-4 backdrop-blur-sm">
            ...
            >
              <div className="mb-6 flex items-center justify-between">
                <h2 className="text-xl font-semibold text-[#5A6670]">
                  {editingId ? "编辑记录" : "新增记录"}
                </h2>
                <button
                  type="button"
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1">
                    ...
                    {item.date && (
                      <p className="text-sm text-[#5A6670]/54">{item.date}</p>
                    )}
                  </div>
                </div>
